...
task :latex_all do
  ...
  main_file=""
  entries.each{|file|
    option={:latex_all=>true}
    next unless file.split('.')[1]=='hiki'
    next if file.include?('.hikirc')
    f_name =File.basename(file,'.hiki')
    if f_name==basename
      option[:latex]="--listings --head latex_dir/head.tex -p "
      main_file=f_name
    else
      option[:latex]="-l 1 --listings -b "
    end
...
def convert_to_latex(file_name, option={})
  ...
  system "hiki2latex #{pre_command} #{option[:latex]} #{tf1} > #{tf2}"
